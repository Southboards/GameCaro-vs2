 ...
 */
 ...
 void View::screenMenuMain()
 {
     ...
     cout << "Enter '3' to Show Rank." << endl;
     cout << "Enter '4' to End." << endl;
 }
...
void View::screenRegister(string &account, string &password)
{
    ...
    if (FileFunction::checkExistAccount(linkPlayerFile))
    {
        ...
        fflush(stdin);
        cin.ignore();
        screenRegister(account, password);
    }
...
void View::screenMenuStart()
{
    ...
    cout << "Enter '2' to Infomation Player." << endl;
    cout << "Enter '3' to Replay Game." << endl;
 ...
 * @param return in this commnad (ref)
 */
void View::screenGetCommandGame(int& command)
{
    clearLine(7);
    clearLine(5);
    clearLine(6);
    ...
    try
    {
        checkInvalid::checkInvalidCommandGame(command_str);
        command = stoi(command_str);
    }
    catch (NumberException& e)
    {
 ...
 */
void View::screenInfoPlayer(string account)
{
    clearLine(1);
    clearLine(2);
    clearLine(3);
    ...
    cout << "Name Player: " << rootObj.value("Name Player").toString().toStdString() << endl;
    cout << "Win: " << FileFunction::getStringNumber(rootObj.value("Win").toInt()) << "  ";
    cout << "Draw: " << FileFunction::getStringNumber(rootObj.value("Draw").toInt()) << "  ";
    cout << "Lose: " << FileFunction::getStringNumber(rootObj.value("Lose").toInt()) << endl;
    cout << "Elo: " << rootObj.value("Elo").toInt() << endl;

    QJsonObject historyObj = rootObj["History Game"].toObject();
    int numGame = historyObj.value("Number of History Game").toInt();
    cout << "History Game: " << endl;
    for (int i=1;i<numGame+1;i++)
    {
        string nameGame = "Game " + to_string(i);
        cout << nameGame << ":" <<endl;
        QJsonObject gameObj = historyObj[nameGame.c_str()].toObject() ;
        cout << "\tDate: " << gameObj.value("Date").toString().toStdString() << endl;
        cout << "\t" << gameObj.value("Player1").toString().toStdString() ;
        cout << " vs ";
        cout << gameObj.value("Player2").toString().toStdString() << endl;
        cout << "\tWinner: " << gameObj.value("Winner").toString().toStdString() << endl;
    }
...
void View::screenReplayGame(string account)
{
    clearLine(1);
    clearLine(2);
    clearLine(3);
    ...
    string numG;
    cout << "Choose Game Replay: " ;
    fflush(stdin);
    getline(cin>>ws, numG);
    int num = stoi(numG);           /// chua check invalid
    if (num>numGame)
    {
        return;
    }
    ...
    string nameGame = "Game " + to_string(num);
    QJsonObject gameObj = historyObj[nameGame.c_str()].toObject() ;
    cout << "\tDate: " << gameObj.value("Date").toString().toStdString() << endl;


    QJsonObject boardSize = gameObj.value("Board").toObject();
    int width = boardSize.value("X").toInt();
    int height = boardSize.value("Y").toInt();
    ...
    QJsonArray listMove = gameObj.value("List Move").toArray();
    COORD move;
    bool turn = PLAYER_1;
    for (int i=0;i<listMove.size();i+=2)
    {
        clearLine(5);
        gotoXY(0,5);
        cout << "\tTurn " ;
        ...
        {
            cout << "(X): " ;
            cout << gameObj.value("Player1").toString().toStdString() << "         " << endl;
        }
        else
        {
            cout << "(O): " ;
        ...
        move.X = listMove[i].toInt();
        move.Y = listMove[i+1].toInt();
